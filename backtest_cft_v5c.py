...
def run_backtest(df):
    ...
    for i in range(1, len(df)):
        ...
        # REGLA CLAVE: en regimen CHOPPY o NEUTRAL, solo operar en direccion del Supertrend
        # Esto evita shorts durante bull runs (ej: dic 2024 con BTC a 100k)
        # y evita longs durante bear markets (ej: 2026 con BTC cayendo)
        if regime in ("choppy", "neutral"):
            if row["h4_st_dir"] == 1:   risk_now = risk_now * 0.7  # solo longs permitidos
            elif row["h4_st_dir"] == -1: risk_now = risk_now * 0.7  # solo shorts permitidos
