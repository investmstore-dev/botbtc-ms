"""
BACKTEST PORTAFOLIO (Opción A) | BTC + DOGE en UNA cuenta compartida
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Simula los dos bots operando sobre el MISMO capital de $10k, con las reglas
de CFT (-10% max DD, -5% daily DD, +8% objetivo) aplicadas al equity COMBINADO.

BTC usa parámetros v5b. DOGE usa la config D optimizada (DD-guard agresivo).

Prueba 2 modos de gestor de riesgo:
  SINGLE : máximo 1 posición a la vez entre ambos pares (conservador)
  DUAL   : ambos pares pueden estar abiertos al mismo tiempo

Reutiliza datos cacheados en data_backtest/.
Uso: python model/backtest_portfolio.py
"""
import os
import warnings
import numpy as np
...
# ── Parámetros por par ───────────────────────────────────────────────────────
PARAMS = {
    "BTCUSDT": dict(risk_trend=0.018, risk_neutral=0.010, risk_choppy=0.005,
                    adx_min=22, adx_choppy=28, dd_lo=0.04, dd_lo_risk=0.008,
                    dd_hi=0.06, dd_hi_risk=0.005),
    "DOGEUSDT": dict(risk_trend=0.014, risk_neutral=0.008, risk_choppy=0.004,
...
def prepare(symbol):
    df = pd.read_csv(os.path.join(CACHE_DIR, f"{symbol.lower()}_h1.csv"),
                     parse_dates=["datetime"]).set_index("datetime").sort_index()
    df = df[["open","high","low","close","volume"]].dropna()
    df_fund = pd.read_csv(os.path.join(CACHE_DIR, f"{symbol.lower()}_funding.csv"),
                          parse_dates=["datetime"]).set_index("datetime").sort_index()
...
def check_entry(row, prev, p, capital, peak):
    """Devuelve dict de posición nueva o None. Mismo criterio v5b."""
    if pd.isna(row["orb_high"]): return None
    if (row["orb_high"]-row["orb_low"])/row["close"] < 0.003: return None
    if not pd.isna(row["atr_avg20"]) and row["atr"]>1.8*row["atr_avg20"]: return None
    chop = row["h4_chop"]
    if chop < 48:    risk = p["risk_trend"]
    elif chop > 57:  risk = p["risk_choppy"];
    else:            risk = p["risk_neutral"]
    regime_choppy = chop > 57
    if regime_choppy and row["adx"] < p["adx_choppy"]: return None
    ...
    if (st_bull and long_sent and orb_up and mom_up and row["macd_hist"]>0 and adx_ok
            and 35 <= row["rsi"] <= 75):
        return {"type":"long", "entry":row["close"], "sl":row["close"]-sl_dist,
                "tp":row["close"]+tp_dist, "lot":lot}
    if (st_bear and short_sent and orb_dn and mom_dn and row["macd_hist"]<0 and adx_ok
            and 25 <= row["rsi"] <= 65):
        return {"type":"short", "entry":row["close"], "sl":row["close"]+sl_dist,
                "tp":row["close"]-tp_dist, "lot":lot}
    return None


def run_portfolio(dfs, mode):
    """dfs: {symbol: df}. mode: 'single' o 'dual'. Capital compartido."""
    capital, peak = INITIAL_CAPITAL, INITIAL_CAPITAL
    positions = {s: None for s in dfs}
    trades = []
    daily_start = {}
    # timeline común
    timeline = sorted(set().union(*[set(df.index) for df in dfs.values()]))
    idx_pos = {s: {ts: k for k, ts in enumerate(df.index)} for s, df in dfs.items()}

    for ts in timeline:
        dt, hour = ts.date(), ts.hour
        daily_start.setdefault(dt, capital)

        # ── 1) Gestionar posiciones abiertas (salidas) ───────────────────────
        for s, df in dfs.items():
            ...
            if hour >= 20:  # EOD
                ...
                trades.append({**pos, "symbol":s, "pnl_usd":pnl, "capital":capital,
                               "exit_reason":"EOD", "exit_date":ts}); positions[s]=None
                continue
        ...
        # ── 2) Guardas CFT sobre equity compartido ───────────────────────────
        ...
        # ── 3) Buscar entradas (respetando el modo) ──────────────────────────
        any_open = any(p is not None for p in positions.values())
        for s, df in dfs.items():
            if positions[s] is not None:
                continue
            if mode == "single" and any_open:
                break
            k = idx_pos[s].get(ts)
            if k is None or k == 0:
                continue
            newpos = check_entry(df.iloc[k], df.iloc[k-1], PARAMS[s], capital, peak)
            if newpos:
                newpos["entry_date"] = ts
                positions[s] = newpos
                any_open = True
                if mode == "single":
                    break
...
def report(t, label):
    ...
    # challenges sobre cuenta compartida
    ...
    print(f"  Max Drawdown    : {dd:+.1f}%   {'[OK CFT]' if abs(dd)<10 else '[VIOLA -10%]'}")
    print(f"  Profit Factor   : {pf:.2f}")
    print(f"  Trades totales  : {n}  |  WR: {wr:.1f}%  |  Días operados: {days}")
    print(f"  Challenges CFT  : {ch}  (días c/u: {ch_days})")
    # contribución por par
    print("  " + "-"*74)
    for s in t["symbol"].unique():
        ts_ = t[t["symbol"]==s]
        w = ts_[ts_["pnl_usd"]>0]
        spf = w["pnl_usd"].sum()/abs(ts_[ts_["pnl_usd"]<=0]["pnl_usd"].sum()) if len(ts_[ts_["pnl_usd"]<=0]) else 999
        print(f"    {s:<9} {len(ts_):>3} trades | P&L ${ts_['pnl_usd'].sum():>+8,.0f} | "
              f"WR {len(w)/len(ts_)*100:>4.0f}% | PF {spf:.2f}")


if __name__ == "__main__":
    print("="*78)
    print("  BACKTEST PORTAFOLIO (Opción A) | BTC v5b + DOGE config-D | cuenta única")
    print("  Periodo: Jul 2024 - Jun 2026 (2 años)")
    print("="*78)
    print("\n  Preparando datos...")
    dfs_full = {s: prepare(s) for s in PARAMS}
    dfs = {s: d[(d.index>="2024-07-01")&(d.index<="2026-06-11")].copy()
           for s, d in dfs_full.items()}

    for mode in ["single", "dual"]:
        label = ("MODO SINGLE (1 posición a la vez entre ambos pares)" if mode=="single"
                 else "MODO DUAL (ambos pares pueden estar abiertos a la vez)")
        report(run_portfolio(dfs, mode), label)

    # Referencia: 2026 solo, ambos modos
    print("\n\n" + "#"*78)
    ...
    dfs26 = {s: d[(d.index>="2026-01-01")&(d.index<="2026-06-11")].copy()
             for s, d in dfs_full.items()}
    for mode in ["single", "dual"]:
        report(run_portfolio(dfs26, mode), f"2026 | {mode.upper()}")
