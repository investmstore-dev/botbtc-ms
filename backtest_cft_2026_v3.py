"""
BACKTEST CFT 2026 v3 | Shorts Mejorados
Mejoras vs v2:
  1. ORB 2 velas de confirmacion (filtra falsos breakouts en mercados choppy)
  2. Filtro de volumen (breakout con volumen > promedio)
  3. Banda de funding para shorts: -0.03% <= funding <= +0.02%
     (evita shorts en squeeze risk Y en mercado bullish)
  4. F&G minimo para shorts: no entrar si F&G < 8 (capitulacion extrema = rebote posible)
  5. Vela de breakout debe ser bajista para shorts (close < open)

Uso: python backtest_cft_2026_v3.py
"""
import pandas as pd
import numpy as np
import os
...
# ── Filtros sentimiento v2 ───────────────────────────────────────────────────
FUND_LONG_MIN     = -0.0002   # no long si funding < -0.02%
FG_LONG_MIN       = 40        # no long si F&G < 40

# ── Filtros SHORT mejorados v3c ───────────────────────────────────────────────
# Solo shortar cuando el mercado tiene sesgo bajista real (funding <= 0)
# Si funding > 0 = participantes pagan para estar largos = no shortar
FUND_SHORT_MIN    = -0.0004   # no short si funding < -0.04% (muy crowded = squeeze risk)
FUND_SHORT_MAX    =  0.00005  # solo short si funding <= ~0 (sesgo bajista o neutral)
FG_SHORT_MIN      = 10        # no short si F&G < 10 (capitulacion pura = rebote posible)
FG_SHORT_MAX      = 65        # no short si F&G > 65 (mercado codicioso)
VOL_MULT_MIN      = 0.8       # volumen breakout >= 80% del promedio 20 periodos
...
# ── Indicadores ───────────────────────────────────────────────────────────────
c, h, l, o = df_raw["close"], df_raw["high"], df_raw["low"], df_raw["open"]
...
# Volumen promedio 20 periodos (para filtro de breakout)
df_raw["vol_avg20"] = df_raw["volume"].rolling(20).mean()
...
df_raw["fg_value"] = pd.Series(
    df_raw.index.date, index=df_raw.index
).map(df_fg["fg_value"].to_dict())
df_raw["fg_value"] = df_raw["fg_value"].ffill()

df_all = df_raw.dropna(subset=["ema500","adx","orb_high","ema200_48h",
                                 "funding_rate","fg_value","vol_avg20"])


# ── Simulacion ────────────────────────────────────────────────────────────────
def run_backtest(df, version="v3"):
    capital     = INITIAL_CAPITAL
    peak        = capital
    trades      = []
    pos         = None
    daily_start = {}
    blocked     = {"fund_long":0, "fg_long":0, "fund_short_high":0,
                   "fund_short_low":0, "fg_short_low":0, "fg_short_high":0,
                   "vol":0, "orb2":0, "body":0}

    for i in range(2, len(df)):   # i>=2 para acceder a i-2
        row   = df.iloc[i]
        prev  = df.iloc[i-1]
        prev2 = df.iloc[i-2]
        dt    = df.index[i].date()
        ...
        # ── ORB breakout (v3c: single-candle, igual que v2) ──────────────────────
        orb_up = (prev["close"] <= row["orb_high"] and row["close"] > row["orb_high"])
        orb_dn = (prev["close"] >= row["orb_low"]  and row["close"] < row["orb_low"])

        # ── Filtro de volumen (breakout con conviccion) ───────────────────────
        vol_ok_short = row["volume"] >= VOL_MULT_MIN * row["vol_avg20"]
        vol_ok_long  = row["volume"] >= VOL_MULT_MIN * row["vol_avg20"]

        # ── Filtros de sentimiento ─────────────────────────────────────────────
        fr = row["funding_rate"]
        fg = row["fg_value"]

        # LONG: no entrar si mercado muy bajista (funding muy negativo o F&G en miedo)
        long_ok_sent = (fr >= FUND_LONG_MIN) and (fg >= FG_LONG_MIN)

        # SHORT v3b: banda de funding + no en capitulacion/euforia
        short_ok_fund = FUND_SHORT_MIN <= fr <= FUND_SHORT_MAX
        short_ok_fg   = FG_SHORT_MIN <= fg <= FG_SHORT_MAX
        short_ok_sent = short_ok_fund and short_ok_fg

        # Contadores para analisis
        if not long_ok_sent and trend_up and mom_up and macro_up:
            if fr < FUND_LONG_MIN: blocked["fund_long"] += 1
            elif fg < FG_LONG_MIN: blocked["fg_long"] += 1
        ...
        # ── LONG ──────────────────────────────────────────────────────────────
        if (long_ok_sent and vol_ok_long and orb_up
                and macro_up and trend_up and mom_up and ema200_up
        ...
        # ── SHORT v3 (con todas las mejoras) ──────────────────────────────────
        elif (short_ok_sent and vol_ok_short and orb_dn
                and macro_dn and trend_dn and mom_dn and ema200_dn
...
def print_report(df_t, label, period_start, period_end, show_trades=True):
    ...
    sw = shorts[shorts["pnl_usd"] > 0]
    sl = shorts[shorts["pnl_usd"] <= 0]
    swr = len(sw) / len(shorts) * 100 if len(shorts) else 0
    spf = sw["pnl_usd"].sum() / abs(sl["pnl_usd"].sum()) if len(sl) and sl["pnl_usd"].sum() != 0 else 999

    lw = longs[longs["pnl_usd"] > 0]
    ll = longs[longs["pnl_usd"] <= 0]
    lwr = len(lw) / len(longs) * 100 if len(longs) else 0
    ...
    print(S2)
    print("  {:<36} {:>14}   {:>8}".format("Trades totales", n, "--"))
    print("  {:<36} {:>14}   {:>8}".format(
        f"  Longs (WR {lwr:.0f}% / PF {lpf:.2f})", f"{len(longs)} ({len(longs)/n*100:.0f}%)", "--"))
    print("  {:<36} {:>14}   {:>8}".format(
        f"  Shorts (WR {swr:.0f}% / PF {spf:.2f})", f"{len(shorts)} ({len(shorts)/n*100:.0f}%)", "--"))
    print("  {:<36} {:>14}   {:>8}".format("Dias operados", trading_days, ">= 5"))
    print("  {:<36} {:>14}   {:>8}".format("Ganadores totales", f"{len(wins)} ({wr:.1f}%)", "--"))
    print("  {:<36} {:>14}   {:>8}".format("  -> Por TP", len(tps), "--"))
    print("  {:<36} {:>14}   {:>8}".format("  -> Por SL", len(sls), "--"))
    print("  {:<36} {:>14}   {:>8}".format("  -> Cierre EOD", len(eods), "--"))
    print("  {:<36} {:>14}   {:>8}".format("Promedio ganancia", f"${aw:,.0f}", "--"))
    print("  {:<36} {:>14}   {:>8}".format("Promedio perdida", f"${al:,.0f}", "--"))
    print("  {:<36} {:>14}   {:>8}".format("Ratio G/P", f"{gpr:.2f}x", ">1.5x"))
    print("  {:<36} {:>14}   {:>8}".format("Profit Factor", f"{pf:.2f}", ">1.2"))
    print("  {:<36} {:>14}   {:>8}".format("Capital final", f"${fin:,.0f}", "--"))
    print("  {:<36} {:>14}   {:>8}".format("Retorno total", f"{ret:+.1f}%", "+8% P1"))
    ...
    if p1_ok and dd_ok and days_ok:
        ...
        if falta > 0:
            print(f"\n  Falta para +8%: ${falta:,.0f} ({ret:+.1f}% de 8%)")
    print(SEP)
    ...
    return {"n": n, "wr": wr, "pf": pf, "ret": ret, "dd": dd_max, "fin": fin,
            ...
            "short_wr": swr, "short_pf": spf}


# ── EJECUTAR ──────────────────────────────────────────────────────────────────
W = 70; SEP = "=" * W
print(SEP)
print("  BACKTEST CFT 2026 v3 | Shorts Mejorados")
print("  ORB 2 velas + volumen + banda funding + F&G min/max")
print(SEP)
...
print(f"  F&G promedio: {df_2026['fg_value'].mean():.0f} (Extreme Fear) | Funding negativo: {(df_2026['funding_rate']<0).sum()/len(df_2026)*100:.0f}%")

print("\n  Ejecutando 2026 v3...")
df_2026_r, blocked_2026 = run_backtest(df_2026.copy())
r_2026 = print_report(df_2026_r, "2026 | v3 (ORB 2 velas + sentimiento mejorado)", "2026-01-01", "2026-06-11")

# 2 anos completos
df_2y = df_all[(df_all.index >= "2024-07-01") & (df_all.index <= "2026-06-11")].copy()
print(f"\n  Velas 2 anos: {len(df_2y):,}")
print("\n  Ejecutando 2 ANOS v3...")
df_2y_r, blocked_2y = run_backtest(df_2y.copy())
r_2y = print_report(df_2y_r, "2 ANOS (Jul 2024 - Jun 2026) | v3", "2024-07-01", "2026-06-11", show_trades=False)

# Por ano
print()
print("  RENDIMIENTO POR ANO")
print("-" * W)
df_2y_r["year"] = pd.to_datetime(df_2y_r["exit_date"]).dt.year
print("  {:<8} {:>8} {:>10} {:>11} {:>8} {:>10}".format("ANO","TRADES","WINRATE","P&L","PF","RET ANUAL"))
print("  " + "-" * 60)
for year, grp in df_2y_r.groupby("year"):
    ...
    sgrp = grp[grp["type"]=="short"]; sw = sgrp[sgrp["pnl_usd"]>0]
    swr = len(sw)/len(sgrp)*100 if len(sgrp) else 0
    print("  {:<8} {:>8} {:>9.1f}% ${:>10,.0f} {:>8.2f} {:>+9.1f}%  {} [S WR:{:.0f}%]".format(
        year, len(grp), len(yw)/len(grp)*100, grp["pnl_usd"].sum(), ypf,
        grp["pnl_usd"].sum()/INITIAL_CAPITAL*100, ok, swr))

# DD maximo 2 anos
eq = [INITIAL_CAPITAL] + df_2y_r["capital"].tolist()
...
for t in df_2y_r.itertuples():
    if not in_ch: in_ch = True; ch_cap = t.capital; ch_pk = t.capital
    ch_pk = max(ch_pk, t.capital)
    if t.capital >= ch_cap * 1.08: challenges += 1; in_ch = False
    elif (t.capital - ch_cap) / ch_cap <= -0.10: in_ch = False

print("=" * W)
print(f"  Retorno 2 anos : {r_2y.get('ret',0):+.1f}%")
print(f"  Capital final  : ${df_2y_r['capital'].iloc[-1]:,.0f}")
print(f"  Max DD 2 anos  : {dd_max_2y:+.1f}%")
print(f"  Challenges CFT : {challenges}")
print(f"  PF 2 anos      : {r_2y.get('pf',0):.2f}")
print(f"  WR 2 anos      : {r_2y.get('wr',0):.1f}%")
print(f"  Short WR 2 anos: {r_2y.get('short_wr',0):.1f}%")
print(f"  Short PF 2 anos: {r_2y.get('short_pf',0):.2f}")
print("=" * W)

# Exportar
df_2026_r.to_csv("test_2026_v3.csv", index=False)
df_2y_r.to_csv("test_2y_v3.csv", index=False)
print(f"\n  Exportado: test_2026_v3.csv  |  test_2y_v3.csv")
print("=" * W)
